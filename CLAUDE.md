...
- Three distinct systems for images/marks — know which one a change belongs to: **motifs** (`src/creative/motifs/`, single-tone vector, theme-recoloured, checked in) · **the curated asset library** (`src/creative/library/`, full-colour raster the user drops in permanently, checked in, searchable via `search_images` as `provider: "library"`, see `src/creative/library/README.md`) · **per-job assets** (`POST /v1/assets`, ephemeral, in the filesystem object store at `STORAGE_DIR`, not theme-recoloured, not searchable later). Don't conflate them — different lifecycles for a real reason.
