...
## Creative library conventions (motifs, and anything added the same way later)

- Hand-authored/curated creative content — anything recolourable, checked into the repo, and meant to grow over time — lives under `src/creative/`, organised into subfolders by subject (see `src/creative/motifs/` — celebration, nature, food, travel, animals, …). Not under `src/components/`; that's rendering code, not library content.
- Every item needs real metadata, not just an id. Motifs use the SVG's own `<title>` (one-liner) plus `<desc>` (when to use it, what it is not, which regions recolour) and `data-tags` — required, no fallback synthesises any of them from the filename. An id-only item is exactly the failure this exists to prevent: something picked by guessing what the name probably means.
- Build the discovery mechanism (search, reference lists) at the same time as the folder, not later "once it's needed." `docs/ROADMAP.md`'s L5 (embeddings/retrieval, gated on "library exceeds ~100 components") is about a *vector* search specifically — a real search function over id/title/tags is not that later-phase item and should exist now for any growing library, especially once the user says they're actively adding more content. Use `search()` in `src/lib/search.ts` (real BM25, not ad-hoc point-scoring, no ML dependency — read that file's header before reaching for embeddings) for any new searchable library; see `searchMotifs` (`src/components/shapes.ts`) and the curated asset library provider (`src/core/images/providers/library.ts`) for the pattern to repeat.
- Never add a downloaded stock/clipart asset (motif, library image, anything) without reading its actual license first, specifically for redistribution-inside-a-product rights — most consumer "personal/commercial use" licenses (Design Bundles, Creative Fabrica, etc.) are single-person licenses that explicitly exclude "a platform that lets end users customize/print their own products," which is what Flyero is. A simple recolour does not usually meet a license's own "transformative work" bar either — check the license's own worked examples, not just the marketing page. New motifs are hand-drawn, or sourced from a license that explicitly permits product redistribution.
- Three distinct systems for images/marks — know which one a change belongs to: **motifs** (`src/creative/motifs/`, vector, theme-recoloured, checked in — single-tone by default, or multiple independently-recolourable regions via `data-tone="ink|accent|accent2|muted|paper|ground"` on every path; never a baked-in colour of its own either way, see `shapes.ts`'s motif-loading header comment) · **the curated asset library** (`src/creative/library/`, full-colour raster the user drops in permanently, checked in, searchable via `search_images` as `provider: "library"`, see `src/creative/library/README.md`) · **per-job assets** (`POST /v1/assets`, ephemeral, in the filesystem object store at `STORAGE_DIR`, not theme-recoloured, not searchable later). Don't conflate them — different lifecycles for a real reason.
