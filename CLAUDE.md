...
- Hand-authored/curated creative content — anything recolourable, checked into the repo, and meant to grow over time — lives under `src/creative/`, organised into subfolders by subject (see `src/creative/motifs/{celebration,nature,communication,objects}/`). Not under `src/components/`; that's rendering code, not library content.
- Every item needs real metadata, not just an id. Motifs use the SVG's own `<title>` element for a one-line description — required, no fallback synthesises one from the filename. An id-only item is exactly the failure this exists to prevent: something picked by guessing what the name probably means.
